      ...
      <Container>
        <Header style = {{ backgroundColor : 'rgb(73, 166, 232)', height : Platform.OS === 'android' ? 70 : 65, paddingTop : Platform.OS === 'android'? 8 : 20}}>
          ...
          <View style = {{ marginTop : Platform.OS === 'android' ? 25 : 10, flex : 1, flexDirection : 'row'}}>
            <Icon style={{ color : '#fff', fontSize:25}} name='menu'/> 
            <Text style ={{ color : '#fff', fontSize : 23, textAlign: 'center', flex : 1}} onPress={this.unsaveUser}>
              Dock 
            </Text>
            <Text style={{paddingRight : 15}}>
            <Icon style={{ color : '#fff', fontSize:25, }} name='search' /> 
            </Text>
            <Icon style={{ color : '#fff', fontSize:25, }} name='bookmark' />
          </View>
        </Header>
        <Content
          refreshControl={
          ...
          <Text style={{fontSize : 15, marginLeft : 12, marginTop : 5}}>
            ...
            <FlatList
              ...
              style = {{marginLeft : 10}}
              showsHorizontalScrollIndicator = {false}
              renderItem={({item}) => <FlatCard image = {'https://mycampusdock.com/' + JSON.parse(item.media)[0]} title = {item.title} channel = {item.title} data = {item} url = {'something'} onPress = {() =>this.props.navigation.navigate('EventDetailScreen', { item })} /> }
            />
          </Content>
      </Container>
    );
